...
class LiveTrader:
    ...
    async def _score_market(self, m: dict) -> dict | None:
        ...
        # Direction hierarchy — always produce a direction (historical data: 50/50 base rate,
        # so any signal > 50% is edge; early entry at ~50¢ only needs weak signal).
        # 1. Price vs open (strongest — this IS what resolves the market)
        # 2. Chainlink (resolution oracle)
        # 3. EMA/Kalman momentum votes (even 1 vote is signal)
        # 4. BTC leads alts by 30-60s — use BTC direction for ETH/SOL/XRP when flat
        # Never skip — always bet, size small on weak signal
        if move_pct >= 0.0003:
            direction = "Up" if current > open_price else "Down"
            if cl_direction and cl_direction != direction and move_pct < 0.0010:
                return None   # conflicting oracles on small move — only real skip
        elif cl_direction:
            direction = cl_direction
        elif tf_up_votes != tf_dn_votes:
            direction = "Up" if tf_up_votes > tf_dn_votes else "Down"
        elif asset != "BTC":
            # BTC leads alts — use BTC price move as direction signal
            btc_price = self.prices.get("BTC", 0) or self.cl_prices.get("BTC", 0)
            btc_open  = next((v for k,v in self.open_prices.items()
                              if self.pending.get(k,(({},{})))[0].get("asset") == "BTC"), 0)
            btc_move  = (btc_price - btc_open) / btc_open if btc_open > 0 and btc_price > 0 else 0
            if abs(btc_move) >= 0.0002:
                direction = "Up" if btc_move > 0 else "Down"
            else:
                direction = "Up" if tf_up_votes >= tf_dn_votes else "Down"  # coin flip → Up bias
        else:
            direction = "Up" if tf_up_votes >= tf_dn_votes else "Down"  # BTC flat → use any momentum
