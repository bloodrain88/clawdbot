...
MAX_ENTRY_PRICE = float(os.environ.get("MAX_ENTRY_PRICE", "0.48"))
...
MIN_PAYOUT_MULT = float(os.environ.get("MIN_PAYOUT_MULT", "1.95"))
...
ENTRY_HARD_CAP_15M = float(os.environ.get("ENTRY_HARD_CAP_15M", "0.52"))
ENTRY_NEAR_MISS_TOL = float(os.environ.get("ENTRY_NEAR_MISS_TOL", "0.020"))
...
MIN_MINS_LEFT_15M = float(os.environ.get("MIN_MINS_LEFT_15M", "2.0"))
EXTRA_SCORE_GATE_BTC_5M = int(os.environ.get("EXTRA_SCORE_GATE_BTC_5M", "1"))
EXTRA_SCORE_GATE_XRP_15M = int(os.environ.get("EXTRA_SCORE_GATE_XRP_15M", "1"))
...
MAX_BOOK_SPREAD_5M = float(os.environ.get("MAX_BOOK_SPREAD_5M", "0.030"))
MAX_BOOK_SPREAD_15M = float(os.environ.get("MAX_BOOK_SPREAD_15M", "0.022"))
    ...
    async def _place_order(self, token_id, side, price, size_usdc, asset, duration, mins_left, true_prob=0.5, cl_agree=True, min_edge_req=None, force_taker=False, score=0, pm_book_data=None, use_limit=False, max_entry_allowed=None, hc15_mode=False, hc15_fallback_cap=0.36):
        ...
        for attempt in range(max(1, ORDER_RETRY_MAX)):
            try:
                ...
                if not cl_agree:
                    edge_floor += 0.02
                spread_cap = MAX_BOOK_SPREAD_5M if duration <= 5 else MAX_BOOK_SPREAD_15M
                if spread > spread_cap and not use_limit:
                    print(
                        f"{Y}[SKIP]{RS} {asset} {side} spread too wide: "
                        f"{spread:.3f} > cap={spread_cap:.3f}"
                    )
                    return None
                ...
                try:
                    ...
                    if fresh_spread > spread_cap:
                        print(
                            f"{Y}[SKIP]{RS} {asset} {side} fallback spread too wide: "
                            f"{fresh_spread:.3f} > cap={spread_cap:.3f}"
                        )
                        return None
